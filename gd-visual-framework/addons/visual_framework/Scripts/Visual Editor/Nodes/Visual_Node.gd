...
signal action_ran(action_name : StringName)
...
@onready var outputs : VBoxContainer = $Body/Outputs
@onready var inputs : VBoxContainer = $Body/Inputs

func _ready() -> void:
	set_existing_node_pins()
...
func set_existing_node_pins() -> void:
	var all_pins = outputs.get_children() + inputs.get_children()
	for pin in all_pins:
		if pin.name.contains("_duplicate") : 
			continue
		var _duplicate = pin.duplicate()
		_duplicate.scene_file_path = ""
		_duplicate.name += "_duplicate"
		pin.get_parent().add_child(_duplicate)
...
func set_node_outputs(output : Variant, node_type = null, node_path : String = "") -> void:
			...
			if node_type == "method":
				if is_instance_of(output, VisualServer.Registered_Method.Tuple):
					...
					for return_argument in method.method_return.properties:
						...
						var node_output_duplicate = node_output.duplicate()
						node_output_duplicate.scene_file_path = ""
						
						node_output_duplicate.output_type = method.method_return.properties[return_argument]["type"]
						node_output_duplicate.set_output_text(return_argument)
						node_output_duplicate.node_owner = self
						outputs.add_child(node_output_duplicate)
					return
	var node_output_path : PackedScene = load("res://addons/visual_framework/Scenes/Visual Editor/Nodes/Node_Output.tscn")
	var node_output : Node_Output = node_output_path.instantiate()
	...
	node_output_duplicate.output_type = typeof(output)
	node_output_duplicate.set_output_text(type_string(typeof(output)))
	outputs.add_child(node_output_duplicate)
	return
